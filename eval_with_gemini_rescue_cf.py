"""
Same as eval_with_gemini_rescue.py, but uses Causal Forcing I2V servers
(cf_server.py) instead of Wan2.2 for candidate video generation.

The CF servers expose the same /generate HTTP API as Wan2.2, so the
parallel-generation logic is identical: N requests are submitted
concurrently to N server instances running on different ports/GPUs.
"""
...
def _cf_generate(port: int, image_path: str, prompt: str, save_path: str) -> bool:
    """POST a generation request to a cf_server.py instance."""
    url = f"http://127.0.0.1:{port}/generate"
    payload = {"image_path": image_path, "prompt": prompt, "save_path": save_path}
    try:
        resp = requests.post(url, json=payload, timeout=600)
        resp.raise_for_status()
        return True
    except Exception as e:
        logging.error(f"[CF port={port}] generation failed: {e}")
        return False
...
def _rescue_select_action(
    ...
    cf_server.py instances (one per port), let Gemini pick the best, and
    return the chosen action chunk.
    """
    ...
    # Draw per-candidate trajectory images and save to disk
    plan_image_paths = []
    ...
    # Parallel CF server requests (one per port)
    tasks = [
        {
            "port": cf_base_port + i,
            "img": plan_image_paths[i],
            "save": str(step_save_dir / f"cf_output_{i}.mp4"),
        }
        for i in range(num_samples)
    ]

    logging.info(f"[Rescue] Launching {num_samples} parallel CF generation requests...")
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_samples) as ex:
        futures = [
            ex.submit(_cf_generate, t["port"], t["img"], task_description, t["save"])
            for t in tasks
        ]
        concurrent.futures.wait(futures)

    valid = [(i, t["save"]) for i, t in enumerate(tasks) if os.path.exists(t["save"])]
    if not valid:
        logging.warning("[Rescue] All CF generations failed; using chunk 0.")
...
class Args:
    ...
    cf_base_port: int = 8010   # CF servers on ports cf_base_port, cf_base_port+1, ...
                    ...
                    if not action_plan:
                        ...
                        if rescue:
                            ...
                            best_actions = _rescue_select_action(
                                ...
                                cf_base_port=args.cf_base_port,
            ...
            for i in range(0, len(clean_images), chunk_size):
                ...
                if len(macro_traj_3d) > 1:
                    imageio.imwrite(
                        ...
                        np.asarray(_draw_projected_trajectory(
                            img=clean_images[i], traj_3d=macro_traj_3d,
                            K=history_K[i], E=history_E[i],
                ...
                if clip_frames:
                    imageio.mimwrite(
                        ...
                        [
                            np.asarray(_draw_projected_trajectory(
                                img=frame, traj_3d=macro_traj_3d,
                                K=history_K[i + j], E=history_E[i + j],
                                orig_res=LIBERO_ENV_RESOLUTION, target_res=args.resize_size,
                            ))
